...
El detector de vareta destinado para el módulo de estimación de longitud, tiene como modelo base un \textit{YOLOv8n} y se entreno bajo los siguientes hiperparámetros:
...
\begin{table}[h]
	...
	\begin{tabular}{c c c c c c c}    
		...
		\textbf{Clase}&\textbf{Imágenes}&\textbf{Objetos encontrados}&\textbf{Precision} &\textbf{Recall}&\textbf{mAP 50}&\textbf{mAP 50-95}\\
		\midrule
		Todas & 10 & 40 & 1.0 & 0.923 & 0.983 & 0.574\\		
		\bottomrule
		\hline
	\end{tabular}
...
Como se puede observar, el modelo sin aumento de datos, consigue detectar un total de 40 varetas en 10 imágenes, donde todas las detecciones fueron correctas. Por otro lado, en promedio, el modelo logra en un 98\% de los casos una alta precisión con sus detecciones cuando se requiere un nivel de confianza igual al 50\%, pero el \textit{mAP} 50-95, indica que solo un 57\% de los casos el nivel de confianza esta entre el 50\% y 95\%. Por este motivo, se decidió aplicar aumento de datos.
...
\begin{table}[h]
	...
	\begin{tabular}{c c c c c c c}    
		...
		\textbf{Clase}&\textbf{Imágenes}&\textbf{Objetos encontrados}&\textbf{Precision} &\textbf{Recall}&\textbf{mAP 50}&\textbf{mAP 50-95}\\
		\midrule
		Todas & 10 & 40 & 0.974 & 0.975 & 0.976 & 0.702\\		
		\bottomrule
		\hline
	\end{tabular}
...
Se observa que con aumento de datos, el modelo mejoró la cantidad de veces que se detecta la clase vareta en un 5.1\% con respecto al modelo sin aumento de datos. Por otro lado, también se incrementó a 70\% la cantidad de casos donde se detecta de forma precisa, esta clase, con un nivel de confianza que va entre  50\% y 95\%.
