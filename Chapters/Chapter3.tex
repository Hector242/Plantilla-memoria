...
Para realizar el conteo total de flores por imagen, primero se posprocesaron las detecciones obtenidas del detector de estados fenológicos, donde se buscaron las centroides de cada \textit{bounding box} y se filtró por los \textit{bounding boxes} que representarán algunas de las siguientes clases: flor abierta, flor cerrada, flor sin petalos e incierto. Luego, a través de un script, se creó una lista de flores detectadas y se utilizaron las centroides tomadas anteriormente para evitar la toma de detecciones que tengan una misma posición. De esta  forma, no se toman en cuenta los duplicados.
